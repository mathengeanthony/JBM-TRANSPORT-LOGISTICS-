...
import { warehouseData, WarehouseCategory } from "../data/warehouseData";
import { Box, Building2, ShieldCheck, ThermometerSnowflake } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { TinyMarquee } from "../components/TinyMarquee";

const icons = [Building2, Box, ShieldCheck, ThermometerSnowflake];

export function WarehousePage() {
  const [filter, setFilter] = useState<WarehouseCategory | "All">("All");
  ...
  const filteredWarehouses = warehouseData.filter(v => {
    const matchesCategory = filter === "All" || v.category === filter;
    const matchesSearch = v.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          v.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          v.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          v.operationalRange.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      <section className="w-full bg-white dark:bg-JBM-black text-JBM-charcoal dark:text-white pt-2 md:pt-4 pb-4 md:pb-8 relative border-b border-gray-200 dark:border-white/10 overflow-hidden">
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 relative z-10">
          <div className="flex justify-between items-center gap-2 md:gap-4">
            <div>
              <div className="flex items-center gap-3 mb-4 md:mb-6">
                ...
                <span className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.3em] text-orange-500">Warehousing & Storage Network</span>
              </div>
          ...
          <div className="mt-6 md:mt-8 relative overflow-hidden rounded-2xl -ml-4 sm:-ml-6 md:-ml-8 p-4 sm:p-6 md:p-8 border border-orange-500/10 shadow-xl max-w-4xl group">
            <div className="absolute inset-0 bg-[url('/images/bg_warehouse.jpg')] bg-cover bg-center opacity-90 transition-opacity duration-700"></div>
            ...
            <div className="relative z-10">
              <p className="text-[10px] md:text-sm text-white drop-shadow-md leading-snug md:leading-normal font-medium">
                Explore our network of secure, scalable, and intelligent warehousing assets. From KRA customs-bonded yards at Mombasa Port to GDP-certified cold facilities at JKIA and high-velocity fulfillment centers along the Nairobi Expressway, easily locate the storage infrastructure tailored to your operational node.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mt-4 md:mt-6 border-t border-gray-200 dark:border-white/10 pt-3 md:pt-6">
            <div className="flex flex-col gap-0 md:gap-1">
              <span className="text-xl md:text-3xl font-extrabold text-orange-500">150K+</span>
              <span className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 leading-tight md:leading-normal">m² Covered Capacity</span>
            </div>
            <div className="flex flex-col gap-0 md:gap-1">
              <span className="text-xl md:text-3xl font-extrabold text-orange-500">95K+</span>
              <span className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 leading-tight md:leading-normal">Pallet Racking Positions</span>
            </div>
            <div className="flex flex-col gap-0 md:gap-1">
              <span className="text-xl md:text-3xl font-extrabold text-orange-500">18</span>
              <span className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 leading-tight md:leading-normal">Strategic Regional Hubs</span>
            </div>
            <div className="flex flex-col gap-0 md:gap-1">
              <span className="text-xl md:text-3xl font-extrabold text-orange-500">24/7</span>
              <span className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 leading-tight md:leading-normal">Live WMS & CCTV Telematics</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-6 md:py-24 max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 w-full">
        ...
        <div className="flex flex-col gap-4 mb-6 md:mb-12">
          ...
          <div className="relative w-full max-w-md">
            ...
            <input 
              type="text" 
              placeholder="Search City, Port, or ICD Node..." 
              value={searchQuery}
          ...
          <div className="flex flex-wrap items-center gap-1.5 md:gap-3">
            {["All", "Bonded & Customs", "Cold Storage", "E-Commerce Fulfillment", "Hazmat & Heavy Yards", "Cross-Docking"].map((cat) => {
              const displayLabel = cat === "All" ? "ALL FACILITIES" : cat.toUpperCase();
              return (
                <button
                  key={cat}
            ...
            >
              ...
              <div className="h-48 sm:h-72 w-full relative overflow-hidden bg-JBM-charcoal">
                ...
                <div className="absolute top-6 left-6 flex flex-wrap gap-2">
                  {warehouse.badges.slice(0, 1).map((badge, idx) => (
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <h3 className="text-2xl font-extrabold uppercase text-white tracking-tight mb-1">{warehouse.name}</h3>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-orange-500">{warehouse.title}</p>
                </div>
              </div>
              ...
              <div className="p-6 md:p-8 flex flex-col flex-grow bg-white dark:bg-transparent">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  {warehouse.specs.slice(0, 2).map((spec, idx) => (
                ...
                <div className="mt-auto flex items-center justify-between border-t border-gray-100 dark:border-white/10 pt-6">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500 group-hover:text-JBM-charcoal dark:group-hover:text-white transition-colors">View Facility Details</span>
